...
%bring in testing data (UNCOMMENT THIS SECTION IF waveform_data_testing.mat
%IS PROVIDED)
% clinical_data_t = load('clinical_data_testing.mat');
% waveform_data_t = load('waveform_data_testing.mat');

%COMMENT THIS SECTION IF waveform_data_testing.mat
%IS PROVIDED
...
Xbasic_t = clinical_data_t.num(:,3:5);%grabs some demographic data for model, Gender, Age, Respitory Comordities
...
for k = 1:length(IDs_unique_t)
    ...
    X_s_wave_add_t = Fs_t([1,3,6,7],IDs_t==IDs_unique_t(k))';
...
for k = 1:length(IDn_unique_t)
    ...
    X_n_wave_add_t = Fn_t([1,3,6,7],IDn_t==IDn_unique_t(k))';
...
[thresh] = test_performance(Phat_test, Y_test)

%YOU ADD YOUR CLASSIFIER HERE!!!! 
% using patient specific classification. If greater than 50% of time in
% sepsis, (i.e. p_hat > 0.5 for more than half the time, sepsis).

alpha = 0.7;
Phat_test_s_t = Phat_test(1:numel(Y_s_t));
Phat_test_n_t = Phat_test(numel(Y_s_t)+1:end);

Y_test_bestguess_s_t = [];
for i = 1:numel(IDs_unique_t)
    indices = (IDs_t == IDs_unique_t(i));
    Phat_focus = Phat_test_s_t(indices);
    Phat_focus_avg = mean(Phat_focus);
    
    % Confidence bounds for phat
    X_focus = X_test(indices,:);
    Phat_UB = 1./(1+exp(-[ones(size(X_focus,1),1) X_focus]*(B+1.96*stats.se)));
    Phat_LB = 1./(1+exp(-[ones(size(X_focus,1),1) X_focus]*(B-1.96*stats.se)));
    ...
    plot(Phat_LB,'g-')
    plot(Phat_UB,'b-')
    legend('Phat', 'Lower Bound', 'Upper Bound');
    titleName = sprintf('Phat for Septic Patient %d', IDs_unique_t(i));
    title(titleName)
    hold off

    if Phat_focus_avg >= alpha % sepsis
        Y_test_bestguess_s_t = [Y_test_bestguess_s_t; ones(numel(Phat_focus),1)];
    else
        Y_test_bestguess_s_t = [Y_test_bestguess_s_t; zeros(numel(Phat_focus),1)];
    end
end

Y_test_bestguess_n_t = [];
for i = 1:numel(IDn_unique_t)
    indices = (IDn_t == IDn_unique_t(i));
    Phat_focus = Phat_test_n_t(indices);
    Phat_focus_avg = mean(Phat_focus);
    ...
    plot(Phat_UB,'b-')
    legend('Phat', 'Lower Bound', 'Upper Bound');
    titleName = sprintf('Phat for Non-Septic Patient %d', IDn_unique_t(i));
    title(titleName)

    hold off
    
    if Phat_focus_avg >= alpha % sepsis
        Y_test_bestguess_n_t = [Y_test_bestguess_n_t; ones(numel(Phat_focus),1)];
    else
        Y_test_bestguess_n_t = [Y_test_bestguess_n_t; zeros(numel(Phat_focus),1)];
    end
end

Y_test_bestguess = [Y_test_bestguess_s_t;Y_test_bestguess_n_t];
