...
parser.add_argument("--X", action='store', help="training data ")
parser.add_argument("--Y", action='store', help="training data result")
parser.add_argument("--testX", action='store', help="testing data")
parser.add_argument("--testY", action='store',help="testing data result")
args = parser.parse_args()


# train model
train_expression = pd.read_csv(args.X,header=0) # train_expression.csv
#train_expression = pd.read_csv('train_expression.csv',header=0)
X = np.array(train_expression.T.iloc[1:,:])
phen_train = pd.read_csv(args.Y,header=0) # phen_train.csv
Y = np.array(phen_train.iloc[:,1:]).reshape(145,)
model = LogisticRegression()
model.fit(X,Y)

# test model
test_expression = pd.read_csv(args.testX, header=0) # test_expression.csv
#test_expression = pd.read_csv('test_expression.csv',header=0)
x = np.array(test_expression.T)[1:]
y_predic = model.predict(x).tolist()
phen_test = pd.read_csv(args.testY,header = 0) # phen_test.csv
